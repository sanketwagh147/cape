...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.isdir(DATA_DIRECTORY):
            raise CommandError("Please, create data folder!")

        df1 = pd.read_csv(os.path.join(DATA_DIRECTORY, 'etf_cape_return_forecast.csv'))
        df2 = pd.read_csv(os.path.join(DATA_DIRECTORY, 'stock_cape_return_forecast.csv'))
        df3 = pd.read_csv(os.path.join(DATA_DIRECTORY, 'acf_yield.csv'))
        df4 = pd.concat([
            pd.read_csv(os.path.join(DATA_DIRECTORY, 'outlier_etf_cape_forecast.csv')),
            pd.read_csv(os.path.join(DATA_DIRECTORY, 'outlier_stock_cape_forecast.csv'))
        ])
        ...
        self.stdout.write(
            self.style.SUCCESS('Successfully imported forecasts')
        )