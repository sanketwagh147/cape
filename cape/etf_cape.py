...
def print_data_date():
    print('Data current as of: {}'.format(
        pd.read_csv('../data/etf_cape_as_of_date.csv').iloc[:, 1:].squeeze()))


def forecast_data():
    df = pd.read_csv(
        '../data/etf_cape_return_forecast.csv', index_col=0)
...
def import_results():
    print('Data current as of: {}'.format(
        pd.read_csv('../data/etf_cape_as_of_date.csv'
                    ).iloc[:, 1:].squeeze()))
    df = pd.read_csv(
        '../data/etf_cape_return_forecast.csv', index_col=0)
    df.index.name = 'TICKER'
...
def chart(etf_ticker, chart_num=1):
    ...
    if chart_num == 1:
        display(Image(
            '../django_apps/website/forecast/static/forecast/images/sample_regression_{}.jpg'.format(index_ticker)))
    elif chart_num == 2:
        display(Image(
            '../django_apps/website/forecast/static/forecast/images/sample_observed_forecast_{}.jpg'.format(index_ticker)))
    elif chart_num == 3:
        display(Image(
            '../django_apps/website/forecast/static/forecast/images/long_term_pe_ratio_{}.jpg'.format(index_ticker)))
    elif chart_num == 4:
        display(Image(
            '../django_apps/website/forecast/static/forecast/images/expected_fwd_return_{}.jpg'.format(index_ticker)))
    else:
